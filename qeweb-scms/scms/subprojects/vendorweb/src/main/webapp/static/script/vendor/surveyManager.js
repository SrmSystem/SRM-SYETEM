...
//	  if(!vali)return false;
	...
	initComponent : function(ctId){//根据状态初始化调查表的控制组件
		...
		if(submitStatus==1 && auditStatus!=0)
...
function addTbRow(tbId,cfgId){
	...
	var rowCk = '<input name="rowCk" type="checkbox"  onchange="tableChecks(\''+tbId+'\')"/>';
	tr+='<td>'+rowCk+'</td>';
...
function tableChecks(tbId){
	//2016-06-03 谌啸林添加判断是否全选
	var $tb = $('#'+tbId);
	var $tbody = $($tb.find('tbody').get(0));
	var $checkedBoxArray = $tbody.find('input[name="rowCk"]:checked');
	if($checkedBoxArray == null || $checkedBoxArray.length<=0){
		$('#tableCheck'+tbId).attr("checked", false);
	}
	else
	{
		var $checkedBoxArrays = $tbody.find('input[name="rowCk"]');
		if($checkedBoxArrays.length==$checkedBoxArray.length)
		{
			$('#tableCheck'+tbId).trigger("click");
		}
		else
		{
			$('#tableCheck'+tbId).attr("checked", false);
