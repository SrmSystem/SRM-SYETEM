...
<html>
	<head>
		<title><spring:message code="vendor.purchasePlanningBoard.purchasePlankanban"/><!-- 采购计划看板 --></title>
		<script type="text/javascript" src="${ctx}/static/script/basedata/common.js"></script>
		<script type="text/javascript" src="${ctx}/static/script/purchasePlanningBoard/purchasePlanningBoard.js"></script> 
	</head>
	
	<body style="margin:0;padding:0;">
		<table id="purchasePlanningBoardDatagrId"
			   data-options="fit:true,method:'post',singleSelect:false,   
			   toolbar:'#purchasePlanningBoardDatagridToolbar',
			   pagination:true,rownumbers:true,pageSize:12,pageList:[12,20]">
			<thead><tr>
			<th data-options="field:'factoryCode'"><spring:message code="vendor.purchasePlanningBoard.factory"/><!-- 工厂 --></th>	
			<th data-options="field:'materialCode'"><spring:message code="vendor.purchasePlanningBoard.material"/><!-- 物料 --></th>
			<th data-options="field:'materialName'" ><spring:message code="vendor.purchasePlanningBoard.materialDescription"/><!-- 物料描述 --></th>
			<th data-options="field:'groupCode'"><spring:message code="vendor.purchasePlanningBoard.purchasingGroup"/><!-- 采购组 --></th>
			<th data-options="field:'plannedTurnaroundDay'"><spring:message code="vendor.purchasePlanningBoard.plannedTurnoverDays"/><!-- 计划周转天数 --></th>
			<th data-options="field:'qualityDay'"><spring:message code="vendor.purchasePlanningBoard.qualityInspectionDays"/><!-- 质量检验天数 --></th>
		...
		<div id="purchasePlanningBoardDatagridToolbar" style="padding:5px;">
			<div>
				<a href="javascript:;" class="easyui-linkbutton" data-options="iconCls:'icon-package_green',plain:true" onclick="purchasePlanningBoard.exportPPB()"><spring:message code="vendor.purchasePlanningBoard.derivation"/><!-- 导出 --></a>
				<a href="javascript:;" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="sycPurchasePlanningBoard()"><spring:message code="vendor.purchasePlanningBoard.synchronizePurchasePlankanban"/><!-- 同步采购计划看板 --></a>
			</div>
			<div>
				<form id="form-ppb-search" method="post">
					<spring:message code="vendor.purchasePlanningBoard.factory"/><!-- 工厂 -->：<input type="text" name="search-LIKE_factoryCode" class="easyui-textbox" style="width:100px;"/>
					<spring:message code="vendor.purchasePlanningBoard.material"/><!-- 物料 -->：<input type="text" name="search-materialCode" class="easyui-textbox" style="width:100px;"/>
					<a href="javascript:;" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onclick="purchasePlanningBoard.search()"><spring:message code="vendor.enquiries"/><!-- 查询 --></a>
	...
	<!-- 同步采购计划看板 -->
 	<div id="win-sync-addPurchasePlanningBoard" class="easyui-window"
		style="width: 300px; height: 200px"
		data-options="iconCls:'icon-add',modal:true,closed:true,buttons:'#dialog-adder-purchasePlanningBoard'">
		<form id="form-sync-addPurchasePlanningBoard" method="post" onkeydown="if(event.keyCode==13)return false;" >
			<table style="text-align: right; padding: 5px; margin: auto;" cellpadding="5">
			<tr>
				<td ><spring:message code="vendor.purchasePlanningBoard.factoryCode"/><!-- 工厂编码 -->:</td>
				<td >
					<input  id ="factoryCode" class="easyui-textbox"  name="factoryCode" value=""  data-options="required:true" />
				</td>
			</tr>
			<tr>
				<td ><spring:message code="purchase.order.MaterialCoding"/><!-- 物料编码 -->:</td>
				<td >
					<input  id ="materialCode" class="easyui-textbox"  name="materialCode" value=""  data-options="required:true" />
				</td>
				<td> <a href="javascript:;" class="easyui-linkbutton" style="width:30px" data-options="iconCls:'icon-search'," onclick="purchasePlanningBoard.sysOpenMaterialPPB()"></a></td>
			</tr>
			</table>
		</form>
		<div id="dialog-adder-purchasePlanningBoard" style="padding:5px;">
			<a href="javascript:;" class="easyui-linkbutton"
				onclick="purchasePlanningBoard.synPurchasePlanningBoard()"><spring:message code="vendor.submit"/><!-- 提交 --></a> <a
				href="javascript:;" class="easyui-linkbutton"
				onclick="$('#form-sync-addPurchasePlanningBoard').form('reset')"><spring:message code="vendor.resetting"/><!-- 重置 --></a>
		</div>
	</div>
	
	 <!--   同步物料的查询 -->
   <div id="win-sysMaterialPPB-detail" class="easyui-window" title="选择物料" style="width:600px;height:450px" data-options="iconCls:'icon-add',modal:true,closed:true">
		<div class="easyui-panel" data-options="fit:true">
			<div id="div_sysMaterial-_detail" style="padding:5px;">
				<div>
					<a href="javascript:;" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="purchasePlanningBoard.sysChoiceMaterial()"><spring:message code="vendor.confirm"/><!-- 确认 --></a>
				</div>
				<div>
	             <a class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true"  onclick="purchasePlanningBoard.addMaterialSys();" ><spring:message code="vendor.purchasePlanningBoard.readClipboardContents"/><!-- 读取剪贴板内容 --></a>
				 <a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="purchasePlanningBoard.insertSys()"><spring:message code="vendor.purchasePlanningBoard.add"/><!-- 添加 --></a>
			     <a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-delete',plain:true" onclick="purchasePlanningBoard.removeitSys()"><spring:message code="vendor.deleting"/><!-- 删除 --></a>
	             <a href="javascript:void(0)" class="easyui-linkbutton" data-options="plain:true" onclick="purchasePlanningBoard.resetMaterialSys()"><spring:message code="vendor.purchasePlanningBoard.emptying"/><!-- 清空 --></a>
				</div>
			</div>
			<table id="datagrid-sysMaterialPPB-list" title="物料列表" class="easyui-datagrid"
				data-options="method:'post',singleSelect:false,idField : 'id',onClickRow: purchasePlanningBoard.onClickRowSys,
				rownumbers:true">
				<thead><tr>
	...
	<script type="text/javascript">
		...
		$(function() {
			//加载表头
			getPurchasePlanBoardHead();
			var searchParamArray = $('#form-ppb-search').serializeArray();
			var searchParams = $.jqexer.formToJson(searchParamArray);
			$('#purchasePlanningBoardDatagrId').datagrid({   
		    	url:'${ctx}/manager/order/purchasePlanningBoard/getList',
		    	queryParams:searchParams,
		    	onLoadSuccess: function(data) {
			    	  if (data.rows.length > 0) {
		                     //调用mergeCellsByField()合并单元格
		                    mergeCellsByField("purchasePlanningBoardDatagrId", 'factoryCode,materialCode,materialName,groupCode,plannedTurnaroundDay,qualityDay,unit');
		                 }
			    }
			});
		});
		...
		function getPurchasePlanBoardHead(){
		    $.ajax({  
		    	url:'${ctx}/manager/order/purchasePlanningBoard/getPurchasePlanBoardHead',
		...
		//同步采购计划看板 
		function sycPurchasePlanningBoard(){
				$('#win-sync-addPurchasePlanningBoard').dialog({
					iconCls:'icon-edit',
					title:'<spring:message code="vendor.purchasePlanningBoard.synchronizePurchasePlankanban"/>'
				});
				$('#form-sync-addPurchasePlanningBoard').form('clear');
				$('#win-sync-addPurchasePlanningBoard').dialog('open');
		}
		     ...
		     for (j = ColArray.length - 1; j >= 0; j--) {
		         ...
		         for (i = 0; i <= TableRowCnts; i++) {
		             if (i == TableRowCnts) {
		                ...
		                CurTxt = tTable.datagrid("getRows")[i].factoryCode+tTable.datagrid("getRows")[i].materialCode+tTable.datagrid("getRows")[i].groupCode; 
		             }
		             if (PerTxt == CurTxt) {
		                 tmpA += 1;
		             }
