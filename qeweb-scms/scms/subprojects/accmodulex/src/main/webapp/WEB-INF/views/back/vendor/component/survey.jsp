				...
				<fieldset>
				  ...
				  <legend class="text-left"><span class="label label-primary"><samll>${ct.title}</samll></span></legend>
					...
					<tr class="datagrid-cell">
					  <th><input type="checkbox" class="tableCheck"/></th>
							...
							<c:forEach items="${tr.tdList}" var="td" varStatus="tdStatus">
								...
								<c:if test="${ct.colList[tdStatus.index].type=='day'}">
								  <span class="textbox">
									...
									pType="tr"
									sum="${ct.colList[tdStatus.index].sum}"
									style="width:140px;line-height: normal;" class="Wdate textbox-text" onFocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'${ct.colList[tdStatus.index].dayFmt}'})" type="text" value="${td}" />
								  </span>
								</c:if>
								<c:if test="${ct.colList[tdStatus.index].type=='file'}">
								   <c:forEach items="${fn:split(td,'/')}" var="fileName" varStatus="fileStatus">
								    ...
								    <c:if test="${exsitFileName!='null' && exsitFileName!=''}"><a href="javascript:;" onclick="File.download('${td}')">${exsitFileName}</a></c:if>
								</c:if>
								<!-- 下载类型开始 -->
