...
var ContractModuleItem = {
	...
	addNewItem : function (type){
		...
		$.post(ctx + '/manager/contractmoduleItem/moduleItem/createModuleItemCode',
				{"moduleId": moduleId, "parentId": parentId, "beforeId": beforeId,"moduleItemId":moduleItemId},function(data){
	...
	copyItem : function (){
		...
		$.ajax({
			url:ctx+'/manager/contractmoduleItem/moduleItem/copyModuleItem',
			type:'POST',
			data:selectedItem,
			dataType : 'json',
	...
	editItem : function(){
		...
		$('#win-item-add').window('open');
		$('#form-item-add').form('load',ctx+'/manager/contractmoduleItem/moduleItem/getModuleItemById/'+selectedId);
	},
	/*
	 * 更新条款
	 */
	/*updateItem : function(itemId){
		...
		$.ajax({
			url:ctx+'/manager/contractmoduleItem/moduleItem/updateModuleItem',
			type:'POST',
			data:item,
			dataType : 'json',
			success:function(data){
	...
	deleteItem : function(){
		...
		if(r){
		$.ajax({
			url :ctx + '/manager/contractmoduleItem/moduleItem/deleteModuleItem',
	...
	move : function(id,moveType){
		...
		$.ajax({
			url:ctx+'/manager/contractmoduleItem/moduleItem/moveModuleItem',
			type:'POST',
	...
	fmt_upAndDown : function(v,r,i){
		return '<a class="wait-up" href="javascript:;" onclick="ContractModuleItem.move('+r.id+',0)"></a><a href="javascript:;" class="wait-down" onclick="ContractModuleItem.move('+r.id+',1)"></a>';
	},		 
	/** 树状grid加载后对上移下移列的渲染 */
