var inFactoryUnqualified ={
	search : function(){
		var searchParamArray = $('#form-inFactoryUnqualified-search').serializeArray();
	...
	searchRf: function(){
		var searchParamArray = $('#form-ifuReportForms-search').serializeArray();
		var searchParams = $.jqexer.formToJson(searchParamArray);
		$('#datagrid-ifuReportForms-list').datagrid('load',searchParams);	
		},
	add : function(){
		$('#id').val("0");
		$('#addform').form('reset')
	...
	addsumbil :function(){
		...
		$('#addform').form('submit',{
			...
			url:ctx+'/manager/qualityassurance/InFactoryUnqualified/updateSave/add', 
			onSubmit:function(){
	...
	impSumbim : function(){
		...
		$('#formImport').form('submit',{
			dataType:"json",
			contentType : 'application/json',
			url:ctx+'/manager/qualityassurance/InFactoryUnqualified/filesUpload', 
			onSubmit:function(){
				var isValid = $(this).form('validate');
				if(!isValid){
					$.messager.progress('close');
	...
	release : function(){
        ...
        $.messager.confirm('提示','确定发布吗？',function(r){
			if(r){
				$.ajax({
					url:ctx+'/manager/qualityassurance/InFactoryUnqualified/publishItem',
					type:'POST',
					data:params,
					dataType:"json",
					contentType : 'application/json',
	...
	calcul : function(){
		$.messager.confirm('提示','确定计算吗？',function(r){
			if(r){
				$.ajax({
					url:ctx+'/manager/qualityassurance/InFactoryUnqualified/calculate',
					type:'POST',
					dataType:"json",
					contentType : 'application/json',
					success:function(data){
