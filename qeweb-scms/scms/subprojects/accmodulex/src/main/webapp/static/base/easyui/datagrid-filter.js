(function($){
	...
	function resizeFilter(target, field){
		...
		ff.each(function(){
			...
			var cell2 = tr.find('td[field="'+name+'2"] .datagrid-cell');
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				if (!div){
					...
					if(fopts.type=="datebox"){
						var input2 = filter.init(div, $.extend({height:24},fopts.options||{}));
						input2.addClass('datagrid-filter').attr('name', field);
						input2[0].filter = filter;
						input2[0].menu = createFilterButton(div, fopts.op);
					}
					if (fopts.options){
						if (fopts.options.onInit){
							fopts.options.onInit.call(input[0], target);
							if(fopts.type=="datebox"){
								fopts.options.onInit.call(input2[0], target);
							}
						}
					} else {
						opts.defaultFilterOptions.onInit.call(input[0], target);
						if(fopts.type=="datebox"){
							opts.defaultFilterOptions.onInit.call(input2[0], target);
						}
					}
