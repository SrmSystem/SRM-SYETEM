...
Factory.operateFmt = function(v,r,i){
	if(r.abolished==0)
	{
		return '<a href="javascript:;" class="easyui-linkbutton" data-options="plain:true" onclick="Factory.edit('+r.id+');">'+$.i18n.prop('vendor.basedataJs.Edit')+'</a>';/*编辑*/
	}
	else
	{
		return '';
...
Factory.vfmt=function(v,r,i){
	return '<button class="btn-link" onclick="Factory.openvfSurvey('+r.id+')">'+$.i18n.prop('vendor.basedataJs.See')+'</button>';/*查看*/
}
//编辑
...
Factory.openvfSurvey = function(id) {// org的ID
	$('#win-openvfSurvey').window({
		title:$.i18n.prop('vendor.basedataJs.BrandOwned')/*'所拥有的品牌'*/
	});
	...
	$.ajax({
		...
		success : function(data){
			...
			if(data.success){
				...
				$.messager.alert($.i18n.prop('vendor.basedataJs.Promit'),data.msg,'error');/*'提示'*/
			}
			}catch (e) {
				$.messager.alert($.i18n.prop('vendor.basedataJs.Promit'),data.msg,'error');/*'提示'*/
			}
...
Factory.selectBrand = function(){
	...
	//	$.messager.alert('标题','请选择品牌','warning');
		$.messager.alert($.i18n.prop('vendor.basedataJs.Title'),$.i18n.prop('vendor.basedataJs.ChoiseBrand'),'warning');
		return false;
	}
	var dataList = [];
...
Factory.removeSelectdBrandFmt = function(v,r,i){
	return '<button class="btn-link" onclick="Factory.removeSelectdBrand('+r.id+')">'+$.i18n.prop('vendor.basedataJs.Moving')+'</button>'/*移除*/
}
//移除已选择的品牌
...
//	$.messager.confirm('提示','确定要废除该工厂吗？',function(r){
	$.messager.confirm($.i18n.prop('vendor.basedataJs.Promit'),$.i18n.prop('vendor.basedataJs.SureCancleFactory'),function(r){
		if(r){
