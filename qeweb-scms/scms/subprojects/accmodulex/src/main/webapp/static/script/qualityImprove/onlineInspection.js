var onlineInspection ={
	search : function(){
		var searchParamArray = $('#form-onlineInspection-search').serializeArray();
	...
	searchRf: function(){
		var searchParamArray = $('#form-oiReportForms-search').serializeArray();
		var searchParams = $.jqexer.formToJson(searchParamArray);
		$('#datagrid-oiReportForms-list').datagrid('load',searchParams);	
		},
	add : function(){
		$('#id').val("0");
		$('#addform').form('reset')
	...
	addsumbil :function(){
		...
		$('#addform').form('submit',{
			...
			url:ctx+'/manager/qualityassurance/OnlineInspection/updateSave/add', 
			onSubmit:function(){
	...
	impSumbim : function(){
		...
		$('#formImport').form('submit',{
			dataType:"json",
			contentType : 'application/json',
			url:ctx+'/manager/qualityassurance/OnlineInspection/filesUpload', 
			onSubmit:function(){
				var isValid = $(this).form('validate');
				if(!isValid){
					$.messager.progress('close');
	...
	release : function(){
        ...
        $.messager.confirm('提示','确定发布吗？',function(r){
			if(r){
				$.ajax({
					url:ctx+'/manager/qualityassurance/OnlineInspection/publishItem',
					type:'POST',
					data:params,
					dataType:"json",
					contentType : 'application/json',
	...
	createEightD : function(id){
		var params = $.toJSON(id);
		 $.messager.confirm('提示','确定发起8D整改吗？',function(r){
				if(r){
					$.ajax({
						url:ctx+'/manager/qualityassurance/OnlineInspection/createEightD',
						type:'POST',
						data:params,
						dataType:"json",
						contentType : 'application/json',
	...
	reportForms : function(){
		$('#oiReportForms').window('open');
	},
	manageFmt : function(v,r,i){
		if(r.totalNumber > 1 && r.eightDStatus != 1){
			return '<a href="javascript:;" class="easyui-linkbutton" data-options="plain:true" onclick="onlineInspection.createEightD('+ r.id +');">发起8D整改</a>';
		}else{
			return '';
		}
	},
	...
	exportExcle : function() {
		$('#form-onlineInspection-search').form('submit',{
			url: ctx+'/manager/qualityassurance/OnlineInspection/exportExcel/',
			success:function(data){
				$.messager.progress('close');
			}
		});
