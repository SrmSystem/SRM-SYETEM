...
(function($) {
	...
	$.messager = {
		...
		confirmEx : function(_2ad, msg, fn, yes, no) {
			if (!yes) {
				yes = $.messager.defaults.ok;
			}
			if (!no) {
				yes = $.messager.defaults.cancel;
			}
			var opts = typeof _2ad == "object" ? _2ad : {
				title : _2ad,
				msg : msg,
		...
		progress : function(_2af) {
			...
			// var
			// opts=$.extend({},{title:"",content:undefined,msg:"",text:undefined,interval:300},_2af||{});
			// modify by pjj,modify default text
			var opts = $.extend({}, {
				title : "",
				content : undefined,
				msg : "",
				text : $.messager.defaults.waiting,
				interval : 300
			}, _2af || {});
			var win = _2a9($
					.extend(
							...
							$.messager.defaults,
							{
								...
								onClose : function() {
									...
									if (_2af != null && _2af.onClose) {// modify
																		// pjj
																		// progress
																		// 默认关闭问题修复
	...
	function _4aa(_4ab) {
		...
		if (bb.length) {
			...
			for (var i = 0; i < bb.length; i++) {
				// bc.append("<a href=\"javascript:void(0)\"
				// class=\"textbox-icon "+bb[i].iconCls+"\" icon-index=\""+i+"\"
				// tabindex=\"-1\"></a>");
				// modify by pjj - support title
				bc
	...
	function _503(_504, _505) {
		...
		function cb() {
			...
			try {
				...
				// modify pjj 修复多个body的情况
				while ($(data).find("body") != null
						&& $(data).find("body").length > 0) {
...
(function($) {
	...
	;
	function _695(target) {
		var grid = $.data(target, "datagrid");
		var opts = grid.options;
		if (opts.idField) {
			return grid.checkedRows;
		} else {
			var rows = [];
	...
	;
	function _698(target, _69a) {
		var grid = $.data(target, "datagrid");
		var dc = grid.dc;
		var opts = grid.options;
		var tr = opts.finder.getTr(target, _69a);
		if (tr.length) {
	...
	;
	function _628(target, _6a0) {
		var _6a1 = $.data(target, "datagrid");
		var opts = _6a1.options;
		opts.finder.getTr(target, _6a1.highlightIndex).removeClass(
				"datagrid-row-over");
		opts.finder.getTr(target, _6a0).addClass("datagrid-row-over");
		_6a1.highlightIndex = _6a0;
	}
	;
	function _62f(target, _6a3, _6a4) {
		var _6a5 = $.data(target, "datagrid");
		var opts = _6a5.options;
		var row = opts.finder.getRow(target, _6a3);
		if (opts.onBeforeSelect.apply(target, _5bc(target, [ _6a3, row ])) == false) {
			return;
		}
		...
		if (!_6a4 && opts.checkOnSelect) {
			checkRow(target, _6a3, true);
		}
		if (opts.idField) {
			_5bb(_6a5.selectedRows, opts.idField, row);
		}
		opts.finder.getTr(target, _6a3).addClass("datagrid-row-selected");
		opts.onSelect.apply(target, _5bc(target, [ _6a3, row ]));
		_698(target, _6a3);
	}
	;
	function unselectRow(target, index, _6a9) {
		var grid = $.data(target, "datagrid");
		var dc = grid.dc;
		var opts = grid.options;
		var row = opts.finder.getRow(target, index);
		if (opts.onBeforeUnselect.apply(target, _5bc(target, [ index, row ])) == false) {
			return;
		}
		...
		if (opts.idField) {
			_5b9(grid.selectedRows, opts.idField, row[opts.idField]);
		}
		opts.onUnselect.apply(target, _5bc(target, [ index, row ]));
	}
	;
	function _6ab(target, _6ad) {
		var grid = $.data(target, "datagrid");
		var opts = grid.options;
		var rows = opts.finder.getRows(target);
		var _6af = $.data(target, "datagrid").selectedRows;
		if (!_6ad && opts.checkOnSelect) {
			checkAll(target, true);
		}
		opts.finder.getTr(target, "", "allbody")
	...
	;
	function _6a6(target, _6b3) {
		var _6b4 = $.data(target, "datagrid");
		var opts = _6b4.options;
		var rows = opts.finder.getRows(target);
		var _6b5 = $.data(target, "datagrid").selectedRows;
	...
	;
	function checkRow(target, _6b9, _6ba) {
		var _6bb = $.data(target, "datagrid");
		var opts = _6bb.options;
		var row = opts.finder.getRow(target, _6b9);
		if (opts.onBeforeCheck.apply(target, _5bc(target, [ _6b9, row ])) == false) {
			return;
		}
		if (opts.singleSelect && opts.selectOnCheck) {
			uncheckAll(target, true);
		...
		if (!_6ba && opts.selectOnCheck) {
			_62f(target, _6b9, true);
		}
		var tr = opts.finder.getTr(target, _6b9).addClass("datagrid-row-checked");
		...
		opts.onCheck.apply(target, _5bc(target, [ _6b9, row ]));
	}
	;
	function uncheckRow(target, _6bd, _6be) {
		var _6bf = $.data(target, "datagrid");
		var opts = _6bf.options;
		var row = opts.finder.getRow(target, _6bd);
		if (opts.onBeforeUncheck.apply(target, _5bc(target, [ _6bd, row ])) == false) {
			return;
		}
		if (!_6be && opts.selectOnCheck) {
			unselectRow(target, _6bd, true);
		}
		var tr = opts.finder.getTr(target, _6bd).removeClass(
		...
		opts.onUncheck.apply(target, _5bc(target, [ _6bd, row ]));
	}
	;
	function checkAll(target, _6c2) {
		var _6c3 = $.data(target, "datagrid");
		...
		var bck = opts.finder.getTr(target, "", "allbody").addClass(
	...
	;
	function uncheckAll(target, _6c5) {
		var _6c6 = $.data(target, "datagrid");
		...
		var dc = _6c6.dc;
		var hck = dc.header1.add(dc.header2).find("input[type=checkbox]");
		var bck = opts.finder.getTr(target, "", "checked").removeClass(
				"datagrid-row-checked").find(
				"div.datagrid-cell-check input[type=checkbox]");
	...
	;
	function beginEdit(target, _6c9) {
		var opts = $.data(target, "datagrid").options;
		var tr = opts.finder.getTr(target, _6c9);
		var row = opts.finder.getRow(target, _6c9);
		if (tr.hasClass("datagrid-row-editing")) {
			return;
		}
		if (opts.onBeforeEdit.apply(target, _5bc(target, [ _6c9, row ])) == false) {
			return;
		}
		...
		validateRow(target, _6c9);
		opts.onBeginEdit.apply(target, _5bc(target, [ _6c9, row ]));
	}
	;
	function endEdit(target, _6cf, _6d0) {
		var grid = $.data(target, "datagrid");
		var opts = grid.options;
		var updatedRows = grid.updatedRows;
		var insertedRows = grid.insertedRows;
		var tr = opts.finder.getTr(target, _6cf);
		var row = opts.finder.getRow(target, _6cf);
		if (!tr.hasClass("datagrid-row-editing")) {
			return;
		}
		if (!_6d0) {
			if (!validateRow(target, _6cf)) {
				return;
			}
			var _6d4 = false;
			var _6d5 = {};
			...
			if (_6d4) {
				if (_5b8(insertedRows, row) == -1) {
					if (_5b8(updatedRows, row) == -1) {
						updatedRows.push(row);
					}
				}
			}
			opts.onEndEdit.apply(target, _5bc(target, [ _6cf, row, _6d5 ]));
		}
		tr.removeClass("datagrid-row-editing");
		_6d9(target, _6cf);
		$(target).datagrid("refreshRow", _6cf);
		if (!_6d0) {
			opts.onAfterEdit.apply(target, _5bc(target, [ _6cf, row, _6d5 ]));
		} else {
			opts.onCancelEdit.apply(target, _5bc(target, [ _6cf, row ]));
		}
	}
	;
	function getEditors(target, _6dc) {
		var opts = $.data(target, "datagrid").options;
		var tr = opts.finder.getTr(target, _6dc);
		var _6dd = [];
	...
	function _6de(_6df, _6e0) {
		var _6e1 = getEditors(_6df, _6e0.index != undefined ? _6e0.index : _6e0.id);
	...
	;
	function _6ca(target, _6e3) {
		var opts = $.data(target, "datagrid").options;
		var tr = opts.finder.getTr(target, _6e3);
		tr.children("td").each(
						function() {
							var cell = $(this).find("div.datagrid-cell");
							var _6e4 = $(this).attr("field");
							var col = _614(target, _6e4);
							if (col && col.editor) {
								var _6e5, _6e6;
		...
		_5e3(target, _6e3, true);
	}
	;
	function _6d9(target, _6eb) {
		var opts = $.data(target, "datagrid").options;
		var tr = opts.finder.getTr(target, _6eb);
		tr.children("td").each(function() {
			var cell = $(this).find("div.datagrid-editable");
			if (cell.length) {
				var ed = $.data(cell[0], "datagrid.editor");
	...
	;
	function validateRow(target, _6ed) {
		var tr = $.data(target, "datagrid").options.finder.getTr(target, _6ed);
		if (!tr.hasClass("datagrid-row-editing")) {
			return true;
		}
	...
	;
	function getChanges(target, _6f1) {
	...
	;
	function deleteRow(target, _6f7) {
		var grid = $.data(target, "datagrid");
		var opts = grid.options;
		var data = grid.data;
		var insertedRows = grid.insertedRows;
		var deletedRows = grid.deletedRows;
		$(target).datagrid("cancelEdit", _6f7);
		var row = opts.finder.getRow(target, _6f7);
		if (_5b8(insertedRows, row) >= 0) {
			_5b9(insertedRows, row);
		} else {
			deletedRows.push(row);
		}
		_5b9(grid.selectedRows, opts.idField, row[opts.idField]);
		_5b9(grid.checkedRows, opts.idField, row[opts.idField]);
		opts.view.deleteRow.call(opts.view, target, _6f7);
		if (opts.height == "auto") {
			_5e3(target);
		}
	...
	;
	function insertRow(target, _6fd) {
		var data = $.data(target, "datagrid").data;
		var view = $.data(target, "datagrid").options.view;
		var insertedRows = $.data(target, "datagrid").insertedRows;
		view.insertRow.call(view, target, _6fd.index, _6fd.row);
		insertedRows.push(_6fd.row);
		$(target).datagrid("getPager").pagination("refresh", {
			total : data.total
		});
	}
	...
	function appendRow(target, row) {
		var data = $.data(target, "datagrid").data;
		var view = $.data(target, "datagrid").options.view;
		var _701 = $.data(target, "datagrid").insertedRows;
	...
	function rejectChanges(target) {
		var grid = $.data(target, "datagrid");
		var opts = grid.options;
		var originalRows = grid.originalRows;
		var insertedRows = grid.insertedRows;
		var deletedRows = grid.deletedRows;
		var selectedRows = grid.selectedRows;
		var checkedRows = grid.checkedRows;
		var data = grid.data;
		function _710(a) {
			var ids = [];
			for (var i = 0; i < a.length; i++) {
		...
		function _711(ids, _712) {
			for (var i = 0; i < ids.length; i++) {
				var _713 = _68f(target, ids[i]);
				if (_713 >= 0) {
					(_712 == "s" ? _62f : checkRow)(target, _713, true);
				}
			}
		}
		;
		...
		var _714 = _710(selectedRows);
		var _715 = _710(checkedRows);
	...
	function _646(target, _717) {
		...
		var _719 = opts.loader.call(target, param, function(data) {
	...
	$.fn.datagrid.methods = {
		...
		endEdit : function(jq, _767) {
			return jq.each(function() {
				endEdit(this, _767, false);
			});
		},
		cancelEdit : function(jq, _768) {
			return jq.each(function() {
				endEdit(this, _768, true);
			});
		},
		getEditors : function(jq, _769) {
			return getEditors(jq[0], _769);
		},
		...
		mergeCells : function(jq, _771) {
			return jq.each(function() {
				mergeCells(this, _771);
			});
		},
		showColumn : function(jq, fieldName) {
			return jq.each(function() {
				var p = $(this).datagrid("getPanel");
				p.find("td[field=\"" + fieldName + "\"]").show();
				$(this).datagrid("getColumnOption", fieldName).hidden = false;
				$(this).datagrid("fitColumns");
			});
		},
		hideColumn : function(jq, fieldName) {
			return jq.each(function() {
				var p = $(this).datagrid("getPanel");
				p.find("td[field=\"" + fieldName + "\"]").hide();
				$(this).datagrid("getColumnOption", fieldName).hidden = true;
				$(this).datagrid("fitColumns");
			});
		},
...
(function($) {
	...
	;
	function _85d(target, _85f) {
		var opts = $.data(target, "datagrid").options;
		var dc = $.data(target, "datagrid").dc;
		if (!dc.body1.is(":empty") && (!opts.nowrap || opts.autoRowHeight)) {
			if (_85f != undefined) {
				var _860 = _861(target, _85f);
				for (var i = 0; i < _860.length; i++) {
					_862(_860[i][opts.idField]);
				}
			}
		}
		$(target).datagrid("fixRowHeight", _85f);
		function _862(_863) {
			var tr1 = opts.finder.getTr(target, _863, "body", 1);
			var tr2 = opts.finder.getTr(target, _863, "body", 2);
			tr1.css("height", "");
			tr2.css("height", "");
	...
	;
	function _86c(target, _86e) {
		var opts = $.data(target, "treegrid").options;
		var tr1 = opts.finder.getTr(target, _86e, "body", 1);
		var tr2 = opts.finder.getTr(target, _86e, "body", 2);
		var _86f = $(target).datagrid("getColumnFields", true).length
				+ (opts.rownumbers ? 1 : 0);
		var _870 = $(target).datagrid("getColumnFields", false).length;
		_871(tr1, _86f);
		_871(tr2, _870);
