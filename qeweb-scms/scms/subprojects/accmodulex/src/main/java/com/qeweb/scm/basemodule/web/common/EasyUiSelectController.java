...
public class EasyUiSelectController {
	...
	@RequestMapping(value = "getGroupTypeMap/{isAll}",method = RequestMethod.POST)
	@ResponseBody
	public List<EasyuiComboBox> getGroupTypeMap(@PathVariable(value="isAll") boolean isAll){
		List<EasyuiComboBox> optionList = new LinkedList<EasyuiComboBox>();
		if(isAll) {
			optionList.add(new EasyuiComboBox("", "---请选择---",true));
		}
		Map<Integer,String> map = OrgType.getGroupTypeMap();
		for(Integer key : map.keySet()){
			EasyuiComboBox option = new EasyuiComboBox(String.valueOf(key), map.get(key));
			optionList.add(option);
		}
