 ...
 *
 */
public class BuyerBoardSAP extends CommonSapFw{

	
	public static List<BuyerBoardEntity> sync(ILogger iLogge, Map<String ,Object> param) throws Exception{
		FileWriter fw = initFw(BuyerBoardSAP.class);
		
		StringBuffer log =new StringBuffer();
		log.append("调用sap接口,采购员看板开始").append(DateUtil.getCurrentDate());
		iLogge.log("调用sap接口，采购员看板开始 "+DateUtil.getCurrentDate()+"\r\n");
		JCoFunction function = null;  
		List<BuyerBoardEntity> res = null;   
	       try{
	    	   
	    	   JCoDestination destination = SAPConn.connect();  
		       iLogge.log("与sap接口建立连接"+"\r\n");
		       res=new ArrayList<BuyerBoardEntity>();
	           ...
	           function = destination.getRepository().getFunction(SapTableStructure.PURCHASE_PLANNING_BOARD_Z_SRM_DELIVERY_SPLIT);  
	            ...
	            JCoTable T_RESULT = function.getTableParameterList().getTable(SapTableStructure.PURCHASE_PLANNING_BOARD_IT_VEN_FORMATED);
	            iLogge.log("调用接口名称 ： "+ SapTableStructure.PURCHASE_PLANNING_BOARD_Z_SRM_DELIVERY_SPLIT +" ; 表名："+SapTableStructure.PURCHASE_PLANNING_BOARD_IT_VEN_FORMATED+"\r\n");
	            if (T_RESULT != null && !T_RESULT.isEmpty()) {
	    			 ...
	    			 iLogge.log("总条数:"+T_RESULT.getNumRows()+"开始转成BuyerBoardEntity\r\n");
	    			 ...
	    			 for (int i = 0; i < T_RESULT.getNumRows(); i++) {
	 					...
	 					String vendorCode = T_RESULT.getString("LIFNR");//供应商
	 					String proportion = T_RESULT.getString("ZB");//占比
	 					String plannedTurnaroundDay = T_RESULT.getString("JHZZT");//计划周转天 
	 					String qualityDay = T_RESULT.getString("ZLJYT");//质量检验天数 
	 					String deliveryFrequency = T_RESULT.getString("SHPL");//质量检验天数 
	 					String transportationDay = T_RESULT.getString("YSTS");//质量检验天数 
	 					...
	 					BuyerBoardEntity entity =new BuyerBoardEntity();
	 					...
	 					entity.setTransportationDay(transportationDay);
	 					entity.setFlag(flag);
	            	...
	            	iLogge.log("IT_VEN_FORMATED 中未获得到数据"+"\r\n");
	            }
	       }catch(Throwable e) {  
	        	...
	        	log.append("调用sap接口，采购员看板结束").append(DateUtil.getCurrentDate());
	        	iLogge.log("调用sap接口，采购员看板结束"+DateUtil.getCurrentDate()+"\r\n");
