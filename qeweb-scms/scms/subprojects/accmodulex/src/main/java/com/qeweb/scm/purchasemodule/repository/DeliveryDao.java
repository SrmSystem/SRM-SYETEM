...
public interface DeliveryDao extends BaseRepository<DeliveryEntity, Serializable>,JpaSpecificationExecutor<DeliveryEntity>{
	...
	@Query("from DeliveryEntity a where a.deliveryCode = ?1 and a.abolished=0")
	public DeliveryEntity findDeliveryEntityByCode(String deliveryCode);
	
	@Query("from DeliveryEntity a where a.order.orderCode = ?1 and a.abolished=0")
	public DeliveryEntity findDeliveryEntityByOrderCode(String orderCode);
	
	@Query("from DeliveryEntity a where a.order.orderCode = ?1 and a.order.col2 = ?2 and a.abolished=0")
	public DeliveryEntity findDeliveryEntityByOrderCodeAndOrgId(String orderCode,String orgId);

	@Query("from DeliveryItemEntity a where a.delivery.id = ?1 and a.material.code = ?2 and a.abolished=0")
	public DeliveryItemEntity getDeliveryItem(long id, String itemCode);
	
	@Query("from DeliveryItemEntity a where a.delivery.order.orderCode = ?1 and a.material.code = ?2 and a.abolished=0")
	public DeliveryItemEntity getDeliveryItemByCode(String poNumber, String itemCode);
	
	@Query("from DeliveryItemEntity a where a.delivery.order.orderCode = ?1 and a.delivery.order.col2 = ?3 and a.material.code = ?2 and a.abolished=0")
	public DeliveryItemEntity getDeliveryItemByCodeAndOrgId(String poNumber, String itemCode, String orgId);
	
	@Query("from DeliveryEntity a where a.deliveryCode = ?1 and a.deliveryType =?2 and a.abolished=0")
	...
	@Query("from DeliveryEntity a where a.col11 = ?1 ")
	public DeliveryEntity findDeliveryEntityByAsnCode(String deliverycode);
