    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (type.equals("code128")) {
                localJBarcode = new JBarcode(Code128Encoder.getInstance(), WidthCodedPainter.getInstance(), BaseLineTextPainter.getInstance());
            ...
            //add by chao.gu 20170814
            localJBarcode.setShowText(false);//不显示图片下面的文字
            if(!StringUtils.isEmpty(h)){
            	localJBarcode.setBarHeight(Double.parseDouble(h));//设置图高
            }
            //add end
