package com.qeweb.scm.basemodule.quartz.service;
...
public class SchedulerServiceImpl implements SchedulerService {
	...
	public static Map<String, JobDetail> jobDetailMap = new HashMap<String, JobDetail>();
	...
	@Override
	public void schedule(String triggername, String jobName, String group, String cronExpression) throws Exception{
		if (isValidExpression(new CronExpression(cronExpression))) {
			...
			scheduler.addJob((JobDetail)SpringContextUtils.getBean(jobName), true);
	...
	public void schedule(String name, String group, CronExpression cronExpression) {
		if (isValidExpression(cronExpression)) {
			...
			trigger.setJobName(SchedulerServiceImpl.jobDetailMap.get(name).getKey().getName());
			trigger.setKey(triggerKey);

			try {
				scheduler.addJob(SchedulerServiceImpl.jobDetailMap.get(name), true);
				if (scheduler.checkExists(triggerKey)) {
					scheduler.rescheduleJob(triggerKey, trigger);
				} else {
					scheduler.scheduleJob(trigger);
	...
	public void schedule(String name, Date startTime, Date endTime, int repeatCount, long repeatInterval, String group) {
		if (this.isValidExpression(startTime)) {
			...
			TriggerKey triggerKey = new TriggerKey(UUID.randomUUID().toString(), group);  
