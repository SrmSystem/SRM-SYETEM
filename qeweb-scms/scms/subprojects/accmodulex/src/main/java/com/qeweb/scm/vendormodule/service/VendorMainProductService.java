...
//		vendorMaterialTypeRelDao.delete(res);
		...
		for (String matTypeId : matTypeIdArray) {
			...
			List<VendorMaterialTypeRelEntity> res=vendorMainProductDao.findByOrgIdAndMaterialTypeId(user.orgId, Long.parseLong(matTypeId));
			if(res==null||res.size()<=0){
				...
				vendorMainProductDao.save(vmtr);
			}
		}
		
		List<VendorMaterialTypeRelEntity> res=vendorMaterialTypeRelDao.findByOrgId(user.orgId);
		for (VendorMaterialTypeRelEntity vendorMaterialTypeRelEntity : res) {
			mainProBui.append(",").append(vendorMaterialTypeRelEntity.getMaterialTypeName());
		}
...
//		vendorMainProductDao.save(venMatTypeList);
	...
	public void delete(List<VendorMaterialTypeRelEntity> list) {
		vendorMaterialTypeRelDao.delete(list);
	}
