...
var OrgManage = {
	...
	openWin : function() {
		...
		$('#datagrid-org-list').datagrid({   
	    	url: ctx + '/manager/admin/org/siteInfo',
	    	onDblClickCell: function(rowIndex){
	    		$(this).datagrid('selectRow', rowIndex);
	    		var selections = $('#datagrid-org-list').datagrid('getSelections');
	    		$('#vendorId').val(selections[0].vendor.id);
	    		$("#vendorName").textbox('setValue', selections[0].vendor.name);
	    		$("#col1").textbox('setValue', selections[0].siteCode);
	    		$("#vendorCode").textbox('setValue', selections[0].vendor.code);
	    		$.ajax({
	    			url: ctx + '/manager/order/goodsrequest/getSenders/' + selections[0].vendor.id,
	    			type:'POST',
	    			dataType:'json',
	    			contentType : 'application/json',
	    			success:function(data){
