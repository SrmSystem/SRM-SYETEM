	...
	public void download(Long id, HttpServletResponse response)
		...
		if (vendorBaseInfo.getProperty() != null) {
			if (vendorBaseInfo.getProperty().equals("1")) {
				vendorBaseInfo.setPropertyText("国企");
			}
			if (vendorBaseInfo.getProperty().equals("2")) {
				vendorBaseInfo.setPropertyText("独资");
			}
			if (vendorBaseInfo.getProperty().equals("3")) {
				vendorBaseInfo.setPropertyText("合资");
			}
			if (vendorBaseInfo.getProperty().equals("4")) {
				vendorBaseInfo.setPropertyText("民营");
			}
		}
		...
		while (entries.hasNext()) {
			...
			{
				vendExcel.setThreeyear(vendExcel.getThreeyear()+Long.parseLong(s.getCol5()));
				vendExcel.setTwoyear(vendExcel.getTwoyear()+Long.parseLong(s.getCol6()));
				vendExcel.setOneyear(vendExcel.getOneyear()+Long.parseLong(s.getCol7()));
			}
			list1.add(vendExcel);
		}
