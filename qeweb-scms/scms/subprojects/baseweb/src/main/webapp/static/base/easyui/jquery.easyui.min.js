...
confirmEx:function(_2ad,msg,fn,yes,no){
	...
	var opts=typeof _2ad=="object"?_2ad:{title:_2ad,msg:msg,fn:fn};
	opts=$.extend({},$.messager.defaults,{content:"<div class=\"messager-icon messager-question\"></div>"+"<div>"+opts.msg+"</div>"+"<div style=\"clear:both;\"/>",buttons:[{text:yes,onClick:function(){
	win.window("close");
	opts.fn(true);
...
var opts=$.extend({},{title:"",content:undefined,msg:"",text:$.messager.defaults.waiting,interval:300},_2af||{});
...
if(_2af!=null && _2af.onClose){//modify pjj progress 默认关闭问题修复
...
bc.append("<a href=\"javascript:void(0)\" class=\"textbox-icon "+bb[i].iconCls+"\" title=\""+bb[i].title+"\" icon-index=\""+i+"\" tabindex=\"-1\"></a>");	
}
}
...
while($(data).find("body")!=null && $(data).find("body").length>0){
...
var val=data[name];
/**##########by Vilon form表单加载添加带.的属性支持，带点的属性（对象.属性）不需要再单独出处理############*/
...
function _511_(data,prefix){
	...
	if(!_512(prefix,val)){
	if(!_513(prefix,val)){
	form.find("input[name=\""+prefix+"\"]").val(val);
	form.find("textarea[name=\""+prefix+"\"]").val(val);
	form.find("select[name=\""+prefix+"\"]").val(val);
	}
	}
}
}
