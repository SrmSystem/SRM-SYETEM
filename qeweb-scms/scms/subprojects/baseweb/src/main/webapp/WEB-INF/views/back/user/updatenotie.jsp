...
.tag button {
    ...
    cursor: pointer;
	 ...
	 <div class="stay-list-left" style="overflow:auto;">
        ...
        <form>
            <div class="sea-div">
                <label class="search-lab">公告名称</label><input type="text" class="hotel-name" id="disHotel" value="${noticeEntity.title }"/>
            </div>
<%--             <div class="sea-div">
            ...
            <div class="sea-div">
                <label class="search-lab">是否评论</label>
                <select id="commentPower"  style="width: 30%;height: 33px;line-height: 33px;font-family: arial;border: 1px solid #dcdcdc;border-radius: 5px;margin-left: 28px;text-indent: 10px;display: block;float: left;">
                	<option value="0" <c:if test="${noticeEntity.commentPower==0 }">selected="selected" </c:if>>允许</option>
                	<option value="1" <c:if test="${noticeEntity.commentPower==1 }">selected="selected" </c:if>>不允许</option>
                </select>
                <label class="search-lab" style="width: 10px"></label>
                <label class="search-lab">是否置顶</label>
                <select id="noticeType"  style="width: 30%;height: 33px;line-height: 33px;font-family: arial;border: 1px solid #dcdcdc;border-radius: 5px;margin-left: 28px;text-indent: 10px;display: block;float: left;">
                	<option value="2" <c:if test="${noticeEntity.noticeType==2 }">selected="selected" </c:if>>不置顶</option>
                	<option value="1" <c:if test="${noticeEntity.noticeType==1 }">selected="selected" </c:if>>置顶</option>
                </select>
            </div>
            <div class="sea-div">
                <label class="search-lab">开始日期</label><input type="text" readonly  id="startDate" style="width: 30%" value="<fmt:formatDate value="${noticeEntity.validStartTime}" pattern="yyyy-MM-dd"/>"/>
                <label class="search-lab" style="width: 10px"></label>
                <label class="search-lab">结束日期</label><input type="text" readonly  id="endDate" style="width: 30%" value="<fmt:formatDate value="${noticeEntity.validEndTime}" pattern="yyyy-MM-dd"/>"/>
                 <a href="javascript:;" class="easyui-linkbutton" onclick="ktuo()">设置永久</a>
            </div>
            <div class="sea-div">
           	    <label class="search-lab">浏览权限：</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            	<a href="javascript:;" class="easyui-linkbutton" onclick="window.parent.lookUser()">选择人员</a>
                <a href="javascript:;" class="easyui-linkbutton" onclick="window.parent.lookR()">选择角色</a>
           		（不选择！系统默认全部）
            </div>
            ...
            <div class="sea-div">
					<textarea id="content1" name="content1" cols="900" rows="300" style="width:100%;height:200px;visibility:hidden;">${noticeEntity.content}</textarea>
					 <a href="javascript:;" class="easyui-linkbutton" style="width: 100px;height:50px;
               			display: block;float: left;margin: 10px 0px 0px 45%;cursor: pointer;" onclick="updtatenotieSSX()">保存并发布</a>
            </div>
        </form>
    </div>
