	...
	<form id="formsd" action="${ctx}/manager/Questionnaire/updatequestionnairePost" method="post">
		<input id="zongTitleid" name="zongTitleid" type="hidden" value="${questionnaire.id }" >
			<input id="zongTitle" name="zongTitle"
				...
				value="${questionnaire.title }" placeholder="问卷标题" onfocus="changt('${questionnaire.id }')"/>
		<!-- top bar -->
		...
		<div id="main" class="clearfix">
							...
							<div class="frame">
								<h2 class="entry-title">
									<input id="subjects${subjects.id }" name="subjects${subjects.id }"
										...
										value="${subjects.title }" placeholder="问卷题目" onfocus="changs('${subjects.id }')" />
								</h2>
								...
								<c:forEach items="${subjects.answers}" var="answers">
									<c:if test="${answers.type==1}">
										<li><input type="radio" style="margin-right: 10px"><input  id="answer${answers.id }" name="answer${answers.id }" value="${answers.title }" placeholder="问卷选项" onfocus="changa('${answers.id }')"/></li>
									</c:if>
									<c:if test="${answers.type==2}">
										<li><input type="checkbox" style="margin-right: 10px"><input  id="answer${answers.id }"  name="answer${answers.id }"   value="${answers.title }" placeholder="问卷选项" onfocus="changa('${answers.id }')"/></li>'
									</c:if>
			...
			<a  id="addtssg1" onclick="sumits()" style="width: 90%;height: 20%;margin-left: 50px" class="easyui-linkbutton" data-options="iconCls:'icon-add'">保存问卷</a>
		</div>
